...
export default function PaymentAnalysisPage({
	...
	const leaseMonthly =
		(carPrice *
			0.6 *
			((leaseRate / 12) * Math.pow(1 + leaseRate / 12, leaseTerm))) /
		(Math.pow(1 + leaseRate / 12, leaseTerm) - 1);

	const financeMonthly =
		(financeAmount *
			((financeRate / 12) * Math.pow(1 + financeRate / 12, financeTerm))) /
	...
	const getRecommendationReasons = (): string[] => {
		...
		if (selectedOption === "lease") {
			reasons.push(
				`Your monthly budget of $${monthlyBudget.toFixed(
					0
				)} aligns well with lower monthly lease payments`
			);
			...
			if (creditNum >= 670) {
				reasons.push(
					"Your good credit score qualifies you for competitive lease rates"
				);
			}
			...
			if (creditNum >= 670) {
				reasons.push(
					`Your credit score of ${creditScore} qualifies you for favorable interest rates`
				);
			}
			reasons.push("Full ownership after 5 years with asset value");
		} else {
			reasons.push(
				"Zero interest payments - save thousands over the loan term"
			);
			...
			<div className="max-w-6xl mx-auto">
				...
				<div className="text-center mb-12">
					...
					<p className="text-[#aaaaaa]">
						Here's why {selectedOptionData.name.toLowerCase()} is the best
						option for you
					</p>
				</div>
				...
				<div className="grid md:grid-cols-3 gap-6 mb-12">
					<Card>
						...
						<CardContent>
							...
							<p className="text-sm text-[#aaaaaa] mt-1">
								{selectedOptionData.term > 0
									? `${selectedOptionData.term} months`
									: "No payments"}
							</p>
						</CardContent>
					</Card>
					...
					<Card>
						...
						<CardContent>
							<p className="text-gray-900">
								{selectedOptionData.interestRate.toFixed(2)}% APR
							</p>
							<p className="text-sm text-[#aaaaaa] mt-1">
								{selectedOptionData.interestRate === 0
									? "No interest"
					...
					<Card>
						...
						<CardContent>
							<p className="text-gray-900">
								${selectedOptionData.totalPaid.toFixed(0)}
							</p>
							<p className="text-sm text-[#aaaaaa] mt-1">
											...
											<td className="py-3 px-4">
												<div className="flex items-center gap-2">
													...
													{option.type === selectedOption && (
														<Badge className="bg-[#d71920] text-white text-xs">
															Best
														</Badge>
													)}
												</div>
				...
				<div className="flex gap-4 justify-center">
					...
					<Button className="bg-[#d71920] hover:bg-[#a01419] text-white px-8">
						Apply Now
					</Button>
				</div>
			</div>
